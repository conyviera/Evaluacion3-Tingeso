...
public class DebtsController {
    ...
    private static final String FIELD_DAMAGE_CHARGE = "damageCharge";
    ...
    public ResponseEntity<Object> assessToolDamage(@PathVariable Long idTool,
            @RequestBody Map<String, Object> payload) {
        try {
            if (!payload.containsKey(FIELD_OUTCOME) || payload.get(FIELD_OUTCOME) == null) {
                return ResponseEntity.badRequest().body("El campo 'outcome' es obligatorio y no puede ser nulo.");
            }
            if (!(payload.get(FIELD_OUTCOME) instanceof String)) {
                return ResponseEntity.badRequest().body("El campo 'outcome' debe ser de tipo String.");
            }
            String outcome = (String) payload.get(FIELD_OUTCOME);

            if (!payload.containsKey(FIELD_DAMAGE_CHARGE) || payload.get(FIELD_DAMAGE_CHARGE) == null) {
                return ResponseEntity.badRequest().body("El campo 'damageCharge' es obligatorio y no puede ser nulo.");
            }
            if (!(payload.get(FIELD_DAMAGE_CHARGE) instanceof Integer)) {
                return ResponseEntity.badRequest().body("El campo 'damageCharge' debe ser un número entero (Integer).");
            }
            int damageCharge = (Integer) payload.get(FIELD_DAMAGE_CHARGE);
